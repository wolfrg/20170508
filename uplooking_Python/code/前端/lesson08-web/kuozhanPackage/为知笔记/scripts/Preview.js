/* global wiz_pageInfo:false, wiz_clipper:false, clipResult:false, wiz_contentPreview:false, Wiz_ContentVeil:false */
function Wiz_ContentPreview() {
    ...
    function nudgePreview(direction) {
        ...
        var previousSibling, tempParent, tempSibling, nextSibling;
        var totoal, startNode, endNode, p, wrapper;
        var i, j, k;
        ...
        switch (direction) {
            case "expand":
                temp = previewElement.parentNode;
                while (temp) {
            ...
            case "shrink":
                if (previewElement.enNudgeDescendToNode) {
                    temp = previewElement.enNudgeDescendToNode;
                ...
                for (i = 0; i < previewElement.children.length; i++) {
            ...
            case "topexpand":
                ...
                tempParent = previewElement;
                tempSibling = previewElement.previousElementSibling;

                // just reverse the tree to find interesting sibling.
                while (tempParent) {
                    tempSibling = tempParent.previousElementSibling;
                    while (tempSibling) {
                ...
                if (previousSibling) {
                    totoal = previousSibling.parentNode.children.length;
            ...
            case "bottomexpand":
                nextSibling = null;
                tempParent = previewElement;
                tempSibling = previewElement.nextElementSibling;

                // just reverse the tree to find interesting sibling.
                while (tempParent) {
                    tempSibling = tempParent.nextElementSibling;
                    while (tempSibling) {
                        if (looksInteresting(tempSibling, previewElement)) {
                ...
                // interesting node found, return all elements between this and seleced element.
                if (nextSibling) {
                    totoal = nextSibling.parentNode.children.length;
    ...
    function computeAlternateBoundingBox(range) {
        ...
        for (i = 0; i < initialRects.length; i++) {
...
//                    xmlhttp.setRequestHeader("Accept", "text/css");
...
//                                inlineStyle.innerText = xmlhttp.responseText.replace(/(\r|\n)/gm,'');
...
//    }
    ...
    function completeImgSrc() {
        try {
            ...
            for (var index = 0, length = imgList.length; index < length; index++) {
                ...
                // convertImgToBase64(imgList[this_index].src, function(dataURL){
                //     imgList[this_index].src = dataURL;
                // });
            }
        } catch (err) {
    ...
    Wiz.Browser.onRequest().addListener(messageHandler);
